...
class CommandScorer(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.encoder_gru  = nn.GRU(hidden_size, hidden_size).to(device)
        self.cmd_encoder_gru  = nn.GRU(hidden_size, hidden_size).to(device)
        self.state_gru    = nn.GRU(hidden_size, hidden_size).to(device)
        self.hidden_size  = hidden_size
        self.state_hidden = torch.zeros(1, 1, hidden_size, device=device)
        self.critic       = nn.Linear(hidden_size, 1).to(device)
        self.att_cmd      = nn.Linear(hidden_size * 2, 1).to(device)
